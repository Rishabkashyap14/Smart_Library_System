...
double Return::Fine()
{
	...
	cout<< asctime(&tm);
	std::ostringstream sql1;
	double x=number_of_days/(60 * 60 * 24);
	if(x>=7)
	{
		x = 10 + (x-7)*2;
		cout<<x;
...
/*	if(x>=7)
	...
	if(rc) 
	{
		...
		fprintf(stderr, "Opened database successfully\n");
	sql2<<"UPDATE TRANSACTIONS set Status='Returned' WHERE User_id="<<userID<<" AND Book_id=(SELECT Book_id FROM BOOKS WHERE Book_name='"<<book_name<<"'); UPDATE BOOKS SET Copies=Copies+1 WHERE Book_name='"<<book_name<<"';";
	command=sql2.str();
	rc = sqlite3_exec(db, command.c_str(), callback, 0, &zErrMsg);   
	if( rc != SQLITE_OK )
	{
